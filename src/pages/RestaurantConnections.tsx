...
import { Globe, Building2, CheckCircle, XCircle, Clock, Search, Filter, Plus, FolderSync as Sync, Eye, Settings } from 'lucide-react';
...
export const RestaurantConnections: React.FC = () => {
  const { user } = useAuth();
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [connectionsData, logsData] = await Promise.all([
        ...
        firebaseService.getSyncLogs(user.supplierId, 50)
      ]);
  ...
  const syncProductsToRestaurant = async (connectionId: string, restaurantId: string) => {
    setSyncing(connectionId);
    try {
      const products = await firebaseService.getProducts(user!.supplierId);
      const availableProducts = products.filter(p => p.isAvailable);
      
      const success = await mainSystemIntegration.syncProductsToRestaurant(
        user!.supplierId,
        restaurantId,
  ...
  const getSyncStatusColor = (status: SyncLog['status']) => {
    switch (status) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Restaurant Connections</h1>
          <p className="text-gray-600">Manage partnerships with restaurants</p>
        </div>
        <button
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{connection.restaurantEmail}</p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-gray-900">
                  {format(new Date(connection.connectionDate), 'MMM dd, yyyy')}
                </span>
              </div>

              {connection.syncSettings.lastSync && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Last Sync:</span>
                  <span className="text-sm font-medium text-gray-900">
                    {format(new Date(connection.syncSettings.lastSync), 'MMM dd, HH:mm')}
                  </span>
                </div>
              )}
              ...
              {connection.connectionStatus === 'active' && (
                <div className="pt-3 border-t flex space-x-2">
                  <button
                    onClick={() => syncProductsToRestaurant(connection.id, connection.restaurantId)}
                    disabled={syncing === connection.id}
                    className="flex-1 bg-blue-600 text-white py-2 px-3 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors disabled:bg-gray-400 flex items-center justify-center space-x-1"
                  >
                    <Sync className="w-4 h-4" />
                    <span>{syncing === connection.id ? 'Syncing...' : 'Sync Products'}</span>
                  </button>
                </div>
              )}
          ...
          {syncLogs.slice(0, 10).map((log) => (
            <div key={log.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
              ...
              <div className="text-right">
                <p className={`text-sm font-medium ${getSyncStatusColor(log.status)}`}>
                  {log.status}
                </p>
                <p className="text-xs text-gray-500">
                  {log.details.itemsSucceeded}/{log.details.itemsProcessed} items
                </p>
              </div>
            </div>
          ))}
...
const ConnectRestaurantModal: React.FC<ConnectRestaurantModalProps> = ({
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      alert('Failed to search restaurants. Make sure the main system is accessible.');
    } finally {
      setSearching(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="font-medium text-blue-900 mb-2">How Restaurant Connections Work</h4>
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>• Once accepted, restaurants can browse your product catalog</li>
              <li>• Receive purchase orders directly in your dashboard</li>
              <li>• Sync your products to their supplier management system</li>
            </ul>
          </div>
        </div>
      </div>
